...
def delete_all():
    ...
    for obj in bpy.data.objects:
        if obj.name.startswith("SLR") or obj.name.startswith("Cube"):
            mesh = bpy.data.meshes[obj.name]
            bpy.data.meshes.remove(mesh)

def create_cam_body():
    cam_body = bpy.data.meshes.new("SLR")
    
    # adding cam_body to scene
    body = bpy.data.objects.new("SLR", cam_body)
    ...
    bm2 = bmesh.new()
    bmesh.ops.create_cube(bm2, size=random.uniform(1.5, 2.5))
    bm2.to_mesh(cube)
    
    # cut out the smaller cube
    bool_one = bpy.data.objects["SLR"].modifiers.new(type="BOOLEAN", name="bool 1")
    bool_one.object = bpy.data.objects["Cube"]
    bool_one.operation = 'DIFFERENCE'
    bpy.ops.object.modifier_apply(
        {"object": bpy.data.objects["SLR"]}, modifier=bool_one.name)
    cube_mesh = bpy.data.meshes["Cube"]
    bpy.data.meshes.remove(cube_mesh)
    
    # triangulate the result to prevent 0 volume
    triang_one = bpy.data.objects["SLR"].modifiers.new(type="TRIANGULATE", name="triang 1")
    bpy.ops.object.modifier_apply({"object": bpy.data.objects["SLR"]}, modifier=triang_one.name)
    
    bpy.data.objects["SLR"].select_set(False)
...
def normalize():
    ...
    bpy.context.view_layer.objects.active = bpy.data.objects["SLR"]
    for obj in bpy.data.objects:
        if obj.name.startswith("SLR"):
            bpy.data.objects["SLR"].select_set(True)
    bpy.ops.object.join()
